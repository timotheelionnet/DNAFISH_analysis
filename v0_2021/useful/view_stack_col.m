...
function plot_yprofile_and_fit(xarr,yarr,z,yrange,stack1,yh,fh,Gaussout)
...
pos(:,1) = xarr;
pos(:,3) = z;
yfit = gauss_integrated3D_position_list(Gaussout, pos);
if 2*yrange+1>ny
...
elseif yarr + yrange > ny && yarr - yrange >= 1
        ...
        Imin = min(yprofile(y-yrange:y+yrange));
        Imax = max(yprofile(y-yrange:y+yrange));
        plot(y-yrange:y+yrange,yprofile(y-yrange:y+yrange),'r',...
            [yarr yarr],[Imin Imax]);
        plot(y-yrange:y+yrange,yfit(y-yrange:y+yrange),'r',...
            [yarr yarr],[Imin Imax]);
end 
...
function plot_zprofile_and_fit(xarr,yarr,z,zrange,stack1,zh,fh,Gaussout)
nz = size(stack1,3);
...
pos(:,1) = xarr;
pos(:,2) = yarr;
zfit = gauss_integrated3D_position_list(Gaussout, pos);
...
clear('nz','zprofile','Imin','Imax','z','pos','zfit');
