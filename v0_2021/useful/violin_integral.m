...
%}
%%
function[h,L,MX,MED,bw]=violin_integral(Y,varargin)

%normalizes the violins to distributions integrals, not maximum
...
if isempty(find(strcmp(varargin,'violin_width')))==0
    vw = varargin{find(strcmp(varargin,'violin_width'))+1}
    if ~isempty(vw)
        if ~isa(vw,'numeric')
            vw = 0.3;
        end
...
for i=1:size(Y,2)
    ...
    du(:,i) = mean(diff(u)); %step size in the kernel estimate (all steps have identical size)
    if max(f/sum(f*du(i)))>mm
        mm = max(f/sum(f*du(i)));
    end    
