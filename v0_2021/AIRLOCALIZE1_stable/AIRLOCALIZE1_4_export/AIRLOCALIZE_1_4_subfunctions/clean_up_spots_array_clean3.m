function [fspots2,fpix2,n_wrong,n_double] = ...
    clean_up_spots_array_clean3(fpix,vox_size,im_size,n_exclusion,numdim)
...
if numdim == 3
    not_wrong = logical(not_wrong.*( fpix(:,3) >= 0 ).*( fpix(:,3) < nz));
end
...
tmp = [ceil(fpix(end:-1:1,1:numdim)),(1:size(fpix,1))'];

if numdim ==3
    spots_stack = convert_coordinates_to_stack3(tmp,nx,ny,nz,'ValMode','value','Size',1,'Additive',0);
else
    spots_stack = convert_coordinates_to_stack3(tmp,nx,ny,'ValMode','value','Size',1,'Additive',0);
end
...
fpix2 = fpix(size(fpix,1)-maxima(:,numdim+1)+1,:);
fpix2 = sort_array_by_col_value(fpix2,Icolnum,'descend');
...
if ndims(stack) == 3
    [nx,ny,nz] = size(stack);
    maxima = zeros(size(pts,1),size(pts,2));
    j=1;
    for i = 1:size(pts,1)
