...
gen_linear_interpol_clean_smallYi(stack,spot_ctr,cutwidth,thickness,ROIsize)
...
if numdim==3 
    ...
    zc = spot_ctr(3);
    bg = generate_bg_region_3D2(stack,xc,yc,zc,cutwidth_xy,cutwidth_z,thickness);
else
    bg = generate_bg_region_2D2(stack,xc,yc,cutwidth_xy,thickness);
end
...
if numdim ==3
    [pl,stack_bg_corr,new_ctr,ROIlimits] = ...
        final_4D_plane_small2(stack,xc,yc,zc,cutwidth_xy,cutwidth_z,thickness,plfit,ROIsize);
else
    [pl,stack_bg_corr,new_ctr,ROIlimits] = ...
        final_3D_plane_small2(stack,xc,yc,cutwidth_xy,thickness,plfit,ROIsize);
end
plmean = mean(pl(:));

end
    
function bg = generate_bg_region_3D2(img,xcenter,ycenter,zcenter,cutwidth_xy,cutwidth_z,thickness)
...
function bg = generate_bg_region_2D2(img,xcenter,ycenter,cutwidth_xy,thickness)
...
function  [pl, stack_bg_corr,new_ctr,ROIlimits] = ...
    ...
    [new_ctr,ROIlimits] = compute_ROI_boundariesYi(stack,[xc,yc,zc],[cutwidth_xy,cutwidth_z],thickness,ROIsize);
    ...
    for xpix=xmin:xmax
        for ypix=ymin:ymax
            ...
            %of the center of the pixel (origin 0,0 at the corner of the image).
            pl(xpix-xmin+1,ypix-ymin+1,zpix-zmin+1) = plfit(1)*(xpix-0.5) + plfit(2)*(ypix-0.5) + plfit(3)*zpix + plfit(4);
            stack_bg_corr(xpix-xmin+1,ypix-ymin+1,zpix-zmin+1) = double(stack(xpix,ypix,zpix)) - pl(xpix-xmin+1,ypix-ymin+1,zpix-zmin+1);
            end
        end
    end
...
function  [pl, stack_bg_corr,new_ctr,ROIlimits] = ...
    ...
    [new_ctr,ROIlimits] = compute_ROI_boundariesYi(stack,[xc,yc],cutwidth_xy,thickness,ROIsize);
    ...
    for xpix=xmin:xmax
        for ypix=ymin:ymax
            ...
            stack_bg_corr(xpix-xmin+1,ypix-ymin+1) = ...
                double(stack(xpix,ypix)) - pl(xpix-xmin+1,ypix-ymin+1);
        end
    end
