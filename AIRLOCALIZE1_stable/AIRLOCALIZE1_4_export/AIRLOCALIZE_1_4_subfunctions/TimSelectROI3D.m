function varargout = TimSelectROI3D(stack,varargin)
%==========================================================================
...
GP.hAxes        = axes;
GP.hImg         = imshow(rescale_stack_int(stack,1),'Parent',GP.hAxes);
GP.nz = size(stack,3);
GP.XLim         = xlim;
GP.YLim         = ylim;
...
GP.hzpos        = uicontrol('Style','edit',         'Parent',gcf,                                       'units','normalized',   'position',[1-5.0*LBoxWidth 0 LBoxWidth 0.2],'String',num2str(1));
GP.hzslider     = uicontrol('Style','slider',       'Parent',gcf,'Max',GP.nz,'Min',1,'Value',1,'SliderStep',[1/(GP.nz-1) 1/(GP.nz-1)],'units','normalized','Position',[1-5.0*LBoxWidth 0 LBoxWidth 0.2]);
...
set(GP.hzpos,       'units','pixels'); q = get(GP.hzpos,    'position'); set(GP.hzpos,    'position',[ q(1)+10  LBoxHeight-25   p(3)  25         ]);
set(GP.hzslider,    'units','pixels'); q = get(GP.hzslider,    'position'); set(GP.hzslider,    'position',[ q(1)-p(3)*4  LBoxHeight+8   p(3)*8  25         ]);
...
set(GP.hzslider,'Callback',@slider_change);
set(GP.hzpos,'Callback',@zpos_change);            
...
function slider_change(src,eventdata)
    GP = guidata(gcf);

    %updating the z-slider and the z info window
    z = get(GP.hzslider,'Value');
    z = round(z);
    set(GP.hzslider,'Value',z);
    set(GP.hzpos,'String',num2str(z));
    
    GP.hSPAPI.replaceImage(rescale_stack_int(GP.stack,z));
    guidata(gcf,GP); 

    clear('z');
    return;

function zpos_change(src,eventdata)
    GP = guidata(gcf);
   
    %updating the z-slider and the z info window
    z = str2num(get(GP.hzpos,'String'));
    z = round(z);
    z = max(min(z,GP.nz) , 1);
    set(GP.hzpos,'String',num2str(z));
    set(GP.hzslider,'Value',z);
