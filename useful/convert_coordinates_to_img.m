function stack = convert_coordinates_to_img(varargin)
...
%to zero or nx/ny
    ...
    coor(:,1) = max(1,min(round(coor(:,1)),nx));
    coor(:,2) = max(1,min(round(coor(:,2)),ny));
...
%to zero or nx/ny/nz
    ...
    coor(:,1) = max(1,min(coor(:,1),nx));
    coor(:,2) = max(1,min(coor(:,2),ny));
    coor(:,3) = max(1,min(coor(:,3),nz));
