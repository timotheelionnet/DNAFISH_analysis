function stack = convert_coordinates_to_stack(varargin)
...
function stack2 = fill_stack_2d(coor,nx,ny,valmode)
% fill in a stack [nx ny nz] with zeros except at positions indicated by coor = [x y z I] 
%or [x y z].
%if input positions falls outside of the size of the array, I change the coordinate
%to zero or nx/ny/nz
    if size(coor,2) == 2
        stack2 = zeros(nx,ny,'uint8');
    elseif size(coor,2) >= 3
        stack2 = zeros(nx,ny,class(coor));
    end
    ...
    for i = 1:size(coor,1)
        if size(coor,2)>=3 && strcmp(valmode,'value')            
            stack2(coor(i,1),coor(i,2))=coor(i,3);
        elseif strcmp(valmode,'ones')  
            stack2(coor(i,1),coor(i,2))=1;
        else
            stack2(coor(i,1),coor(i,2))=1;
        end
    end
