function [stack, img_read, filename] = read_2_tiffs()
...
if (nargin == 0)
    [filename1, pathname1] = uigetfile('*.tif;*.stk;*.lsm', 'select image file');
    filename1 = [ pathname1, filename1 ];
    [filename2, pathname2] = uigetfile('*.tif;*.stk;*.lsm', 'select image file');
    filename2 = [ pathname2, filename2 ];
end

img_first = 1; img_last = 10000; 

% tags used only internally can be stored in TIF.
global TIF1;
TIF1 = [];
...
% set defaults values :
TIF1.SampleFormat     = 1;
TIF1.SamplesPerPixel  = 1;
TIF1.BOS              = 'l';          %byte order string
TIF2.SampleFormat     = 1;
TIF2.SamplesPerPixel  = 1;
TIF2.BOS              = 'l';          %byte order string

if  isempty(findstr(filename1,'.'))
    filename = [filename1,'.tif'];
end

if  isempty(findstr(filename2,'.'))
    filename = [filename2,'.tif'];
end
...
while (ifd_pos ~= 0)
    ...
    %% total number of bytes per plane:
...
clear('strip','line','plane','classname','nz');
